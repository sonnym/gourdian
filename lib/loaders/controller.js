...
ControllerLoader.prototype.run = function(route, session, request, response) {
  var action = this.get(route, session, request);
 ...
 * @api public
 */
ControllerLoader.prototype.get = function(route, session, request, socket, message, sockets) {
  ...
  controller_instance.session = session;
