...
module.exports = Server = function(logfile, port) {
  if (logfile) Logger.location = logfile;
...
Server.prototype.start = function() {
  Logger.info("Starting server on port " + this._port + " serving: " + this._configuration.base_path);
...
Server.prototype.stop = function() {
  Logger.info("Stopping server serving base path: " + this._configuration.base_path + " on port: " + this._port);
...
Server.prototype.setup_socket_server = function() {
  ...
  this._io.configure(function () {
    self._io.set("transports", ["jsonp-polling", "websocket", "htmlfile", "xhr-polling"]);
...
Server.prototype.start_http_server = function() {
  ...
  this._http_server.on("request", function(request, response) {
    Logger.info("HTTP request: " + JSON.stringify(request.url));
  });

  var self = this;
  this._http_server.listen(this._port, function() {
    Logger.info("HTTP server successfully bound to port: " + self._port);
