...
var Server = function(logfile, port, base_path) {
  ...
  else this.base_path = Gourdian.ROOT;
};
...
function start_http_server(base_path, port) {
  ...
  var file_server;
  if (router.root || (router.files && router.files.length > 0)) {
    var public_path = path.join(base_path, router.root.root);
    Gourdian.logger.info("Starting static file server at: " + base_path)

    file_server = new static_handler.Server(base_path);
  } else {
  ...
  var http_server = require("http").createServer(function(request, response) {
    ...
    request.addListener("end", function() {
      var action_route = router.lookup_action_route(request.url)
        , file_route = router.lookup_file_route(request.url)
        , in_root = router.in_root(request.url);
      ...
      if (file_server && (file_route || in_root)) {
        if (file_route) {
          ...
          var path_to_serve = path.join(router.root.root, url.parse(request.url).pathname);
