  ...
  this._http_server = http.createServer(function(request, response) {
    ...
    request.on("end", function() {
      ...
      if (file_server && in_root) {
        ...
        file_server.serve(request, response, function() { response.end() });
...
Server.prototype.setup_socket_server = function() {
  ...
  var self = this
    ...
    , socket_route_events = socket_routes.events ? Gourdian._.keys(socket_routes.events) : [];
  ...
  this._io.sockets.on("connection", function(socket) {
    ...
    for (var i = 0, l = socket_route_events.length; i < l; i++) {
      ...
      socket.on(key, (function() {
        var k = key;
        return function() {
          var result = self._controller_loader.get(socket_routes.events[k], session, "", socket)();
    ...
    if (socket_routes.message) socket.on("message", function(msg) {
      var result = self._controller_loader.get(socket_routes.message, session, "", socket, msg)();
      if (result && typeof result === 'string') socket.send(result);
    });
