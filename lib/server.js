  ...
  this._io.sockets.on("connection", function(socket) {
    ...
    for (var i = 0, l = socket_route_events.length; i < l; i++) {
      ...
      socket.on(key, (function() {
        var k = key;
        return function() {
          var result = self._controller_loader.get(socket_routes.events[k], session)();
          socket.send(result);
        };
      })());
    }
    ...
    if (socket_routes.message)  socket.on("message", function(msg) {
      var result = self._controller_loader.get(socket_routes.message, session, "", socket.id, msg)();
      socket.send(result);
    });
