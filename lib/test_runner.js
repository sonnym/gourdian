...
module.exports = TestRunner = function() {
  ...
  // settings
  this._server_path = Gourdian.ROOT;
...
/* @param {String} v absolute path to application directory */
TestRunner.prototype.__defineSetter__("server_path", function(v) { this._server_path = v });
...
TestRunner.prototype.run = function() {
  while (this.running <= this._max_running && this.pending > 0) {
    ...
    if (this._filters.name && test_name !== this._filters.name) {
      ...
      if (test_instance.start_server) test_instance.start_server(this._server_path);
      try {
        this._counts.t++;
