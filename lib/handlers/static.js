 ...
 * @api public 
 */
StaticHandler.prototype.init = function(router, base_path) {
  if (!router.root) {
  ...
  this._public_path = path.join(base_path, router.root.root);
...
StaticHandler.prototype.handles = function(request) {
  return this._file_server && this._router.in_root(request.url);
};
