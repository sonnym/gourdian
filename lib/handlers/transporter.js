...
TransporterHandler.prototype.init = function() {
  this.share_path = path.join(Gourdian.Configuration.base_path, "app", "share")
  this._transporter = new Transporter({paths: [this.share_path]});
};
...
TransporterHandler.prototype.handles = function(request) {
  ...
  return this._transporter && fs.existsSync(path.join(this.share_path, last_part));
};
