...
module.exports.r_cp = function(from, to, overwrite_existing_files, callback) {
  ...
  module.exports.directory_descent_wrapper(from, function(source) {
    var source_rel = source.substring(from.length + 1)
      ...
      , destination = path.join(to, Gourdian._.reduce(source_rel_array, function(memo, path_part) { return path.join(memo, path_part) }));

    calls++;
    fs.stat(source, function(err, stat) {
      if (err) Gourdian.logger.debug("Error copying file: " + source + "; " + err.message);
...
module.exports.reduce_directory_structure = function(root_path, structure) {
  return Gourdian._.flatten(Gourdian._.map(structure, function(iterated_object) {
    if (typeof iterated_object === "object") {
      var key = Gourdian._.keys(iterated_object)[0];
      return [ path.join(root_path, key), module.exports.reduce_directory_structure(path.join(root_path, key), Gourdian._.values(iterated_object)[0]) ];
