var ib = function() {
  ...
  function valid_locations(board, p) {
    ...
    if (piece == "p") {
      ...
      valid = multiplicative_checker(start, [7, 9])
    } else if (piece == "N" || piece == "n") {
      valid = multiplicative_checker(start, [6, 10, 15, 17], 1)
    } else if (piece == "R" || piece == "r") {
      valid = multiplicative_checker(start, [8])
    } else if (piece == "Q" || piece == "q") {
      $.merge(valid, multiplicative_checker(start, [7, 8, 9]));
    } else if (piece == "K" || piece == "k") {
      valid = multiplicative_checker(start, [7, 8, 9], 1)
    }

    return valid;
  }

  function multiplicative_checker(start, distances, depth) {
    var valid = [],
    ...
    for (var d in distances) {
      var current = 1;
      do {
        var indices = [start + (distances[d] * current), start - (distances[d] * current)];
        for (var i in indices) {
          if (indices[i] < 64 && indices[i] >= 0) valid.push(indices[i]);
        }
        current += 1;
      } while(iter(current, distances[d]) && (!depth || current <= depth))
    }
